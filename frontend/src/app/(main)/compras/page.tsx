...
const statusBadge: Record<PurchaseOrder["estatus"], string> = {
...
export default function ComprasPage() {
    ...
    const totalPendiente = MOCK_PURCHASES.filter(p => p.estatus === "PENDIENTE").reduce((a, p) => a + p.total, 0)
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3">
                <Card><CardContent className="pt-4 pb-4 px-4">
                    <div className="text-xl font-bold">{MOCK_PURCHASES.filter(p => p.estatus === "PENDIENTE").length}</div>
                ...
                <Card><CardContent className="pt-4 pb-4 px-4">
                    <div className="text-xl font-bold">{MOCK_PURCHASES.length}</div>
                            ...
                            {filtered.map(p => (
                                <tr key={p.id} className="hover:bg-muted/30 transition-colors cursor-pointer">
                                    ...
                                    <td className="px-4 pr-5 py-3.5">
                                        <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium border ${statusBadge[p.estatus]}`}>
                                            {p.estatus.charAt(0) + p.estatus.slice(1).toLowerCase()}
                                        </span>
                                    </td>
                                </tr>
                            ))}
